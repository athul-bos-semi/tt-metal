...
namespace ttnn::operations::test_ops {
    ...
    Concat::cached_program_t Concat::MultiCore::create(const operation_attributes_t &operation_attributes, const tensor_args_t &tensor_args,Tensor &output) {
        ...
        return { 
            std::move(program), {.unary_reader_kernel_id = unary_reader_kernel_id, .unary_writer_kernel_id = unary_writer_kernel_id, .num_input_tensors = num_input_tensors, .num_output_rows_per_core = num_output_rows_per_core, .output_stick_size = output_stick_size} 
        };
    }
